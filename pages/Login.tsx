...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        await setDoc(doc(db, 'users', cred.user.uid), { name, email, role: 'Student', createdAt: new Date().toISOString() });
      } else {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', result.user.uid), { name: result.user.displayName, email: result.user.email, role: 'Student', createdAt: new Date().toISOString() }, { merge: true });
      navigate('/home');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 pt-20 pb-12">
      <MotionDiv initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6 }}
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-display font-bold text-white">{isRegister ? 'Create Account' : 'Welcome Back'}</h1>
          <p className="text-sm text-slate-400 mt-1">{isRegister ? 'Join E-Prayog today' : 'Log in to your E-Prayog account'}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? <span className="animate-spin w-4 h-4 border-2 border-white/30 border-t-white rounded-full" /> : <LogIn size={16} />}
            {isRegister ? 'Create Account' : 'Log In'}
          </button>
        </form>

        <div className="flex items-center my-6"><div className="flex-1 h-px bg-white/10" /><span className="px-4 text-xs text-slate-500">OR</span><div className="flex-1 h-px bg-white/10" /></div>
        ...
        <button onClick={handleGoogle} disabled={loading}
          ...
          <svg width="18" height="18" viewBox="0 0 48 48"><path fill="#FFC107" d="M43.6 20.1H42V20H24v8h11.3C34 32.7 29.5 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.2 8 3l5.7-5.7C34 6 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.2-2.7-.4-3.9z"/><path fill="#FF3D00" d="m6.3 14.7 6.6 4.8C14.2 16.2 18.8 13 24 13c3.1 0 5.8 1.2 8 3l5.7-5.7C34 6 29.3 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"/><path fill="#4CAF50" d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.2 35.1 26.7 36 24 36c-5.5 0-10-3.3-11.3-8.1l-6.5 5C9.5 39.6 16.2 44 24 44z"/><path fill="#1976D2" d="M43.6 20.1H42V20H24v8h11.3c-.8 2.2-2.2 4.2-4.1 5.6l6.2 5.2C37 39.2 44 34 44 24c0-1.3-.2-2.7-.4-3.9z"/></svg>
          Continue with Google
        </button>

        <p className="text-center text-sm text-slate-400 mt-6">
          {isRegister ? 'Already have an account?' : "Don't have an account?"}
          <button onClick={() => { setIsRegister(!isRegister); setError(''); }} className="text-emerald-400 font-bold ml-2 hover:underline">
